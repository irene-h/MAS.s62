...
func EZTxBuilder(sendfromtxid string, out uint32, sendtoadd string, pk string, sendfromadd string, qty int64) *wire.MsgTx {
	...
	hashStr := sendfromtxid // put the input txid here
	...
	outPoint := wire.NewOutPoint(
		outpointTxid, out) // replace 0 with the output you want to spend
	...
	sendToAddressString := sendtoadd // put the address you're sending to here
	...
	output := wire.NewTxOut(qty, sendToScript) // replace 0 with the output value
	...
	phraseHash := chainhash.DoubleHashB([]byte(pk))
	...
	prevAddressString := sendfromadd // put the address you're sending "from" here
