...
func Forge() (string, Signature, error) {
	...
	// complete private key with revealed values
	var sk SecretKey

	for j := 0; j < 4; j++ {
		for i := 0; i < 256; i++ {
			if msgslice[j][i/8]>>uint(7-(i%8))&0x01 == 1 {
				sk.OnePre[i] = sigslice[j].Preimage[i]
			} else {
				sk.ZeroPre[i] = sigslice[j].Preimage[i]
			}
		}
	}

	// gather what blocks have not been revealed
	...
	for i := 0; i < 256; i++ {
		if sk.OnePre[i] == c {
			u.one = append(u.one, i)
		}
		if sk.ZeroPre[i] == c {
			u.zero = append(u.zero, i)
		}
	}

	// try sign message	until verification works

	nonce := 1
	worked := false
	var sig Signature
	msgString := "forge - from Irene Hernandez "

	for worked == false {
		msgString = fmt.Sprintf("%s%d", "forge - from Irene Hernandez ", nonce)
